...
import { updateUserInfo, Userprojects, getUserProfile } from '../services/ProfileUpdate';
...
export const Profile = () => {
  ...
  useEffect(() => {
    if (isMyProfile) {
      ...
      if (currentUser && currentUser.users_info) {
        setProfileData(currentUser);
        setJob(currentUser.users_info.job || '');
        setPhone(currentUser.users_info.phone || '');
        setAddress(currentUser.users_info.address || '');
        setCity(currentUser.users_info.city || '');
        if (currentUser.users_info.background) {
          setBackground(`${import.meta.env.VITE_STORAGE_URL}/${currentUser.users_info.background}`);
        }
        if (currentUser.users_info.profile_photo) {
          setProfilePic(`${import.meta.env.VITE_STORAGE_URL}/${currentUser.users_info.profile_photo}`);
        }
      }
    } else {
      ...
      const fetchProfile = async () => {
        try {
          ...
          if (response.data.user) {
            ...
            if (fetchedUser.users_info) {
              setJob(fetchedUser.users_info.job || '');
              setPhone(fetchedUser.users_info.phone || '');
              setAddress(fetchedUser.users_info.address || '');
              setCity(fetchedUser.users_info.city || '');
              if (fetchedUser.users_info.background) {
                setBackground(`${import.meta.env.VITE_STORAGE_URL}/${fetchedUser.users_info.background}`);
              }
              if (fetchedUser.users_info.profile_photo) {
                setProfilePic(`${import.meta.env.VITE_STORAGE_URL}/${fetchedUser.users_info.profile_photo}`);
              }
            }
          }
        } catch (error) {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const response = await Userprojects(routeUsername); 
              ...
              <div className="flex items-start justify-between mb-8">
                <div>
                  ...
                  <p className="text-gray-500">
                    {profileData?.users_info?.job || 'Senior Product Designer'}
                  </p>
                </div>
                {isMyProfile && (
                  <button
